...
describe PDF::Reader2::Buffer, "token method" do
  ...
  context "indirect reference" do
    it "tokenises correctly" do
      ...
      expect(buf.token).to eql("aaa")
      expect(buf.token).to be_a_kind_of(PDF::Reader2::Reference)
      expect(buf.token).to eql("bbb")
      expect(buf.token).to be_nil
    end
  end
  ...
  context "initialize with a specific position" do
    it "tokenises correctly" do
      str = "aaa bbb ccc"
      buf = PDF::Reader2::Buffer.new(StringIO.new(str), :seek => 4)
  ...
  context "initialize with a specific position" do
    it "tokenises correctly" do
      str = "aaa bbb ccc"
      buf = PDF::Reader2::Buffer.new(StringIO.new(str), :seek => 5)
  ...
  context "inline image when inside a content stream" do
    it "tokenises correctly" do
      ...
      buf = PDF::Reader2::Buffer.new(io, :content_stream => true)
  ...
  context "inline image" do
    context "outside a content stream" do
      it "tokenises correctly" do
        io = StringIO.new(binary_string("BI ID aaa bbb ccc \xF0\xF0\xF0 EI"))
        buf = PDF::Reader2::Buffer.new(io, :content_stream => false)
  ...
  context "inline image without a trailing 'EI'" do
    context "inside a content stream" do
      it "tokenises correctly" do
        ...
        expect {
          buf.token
        }.to raise_error(PDF::Reader2::MalformedPDFError, "EI terminator not found")
      end
    end
  end
...
describe PDF::Reader2::Buffer, "empty? method" do
  include BufferHelper
...
describe PDF::Reader2::Buffer, "find_first_xref_offset method" do
  ...
  context "when buffer doesn't contain an EOF marker" do
    it "raises an exception" do
      ...
      expect { @buffer.find_first_xref_offset }.to raise_error(PDF::Reader2::MalformedPDFError)
    end
  end
...
describe PDF::Reader2::Buffer, "read method" do
  include BufferHelper
