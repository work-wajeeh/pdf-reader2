...
describe PDF::Reader2::OverlappingRunsFilter, "#exclude_redundant_runs" do

  let(:result) {
    PDF::Reader2::OverlappingRunsFilter.exclude_redundant_runs(runs)
  }
  ...
  context "when there's two identically positioned runs with different text" do
    let(:runs) do
      [
        PDF::Reader2::TextRun.new(30, 700, 50, 12, "Hello"),
        PDF::Reader2::TextRun.new(30, 700, 50, 12, "Bacon"),
      ]
    end
  ...
  context "when the second run overlaps the right edge of the first" do
    context "by 50%" do
      let(:runs) do
        [
          PDF::Reader2::TextRun.new(30, 700, 50, 12, "Hello"),
          PDF::Reader2::TextRun.new(55, 700, 50, 12, "Hello"),
        ]
      end
      it "returns only one of the runs" do
        expect(result).to match_array(runs.slice(0,1))
    ...
    context "by less than 50%" do
      let(:runs) do
        [
          PDF::Reader2::TextRun.new(30, 700, 50, 12, "Hello"),
          PDF::Reader2::TextRun.new(65, 700, 50, 12, "Hello"),
        ]
      end
      it "returns only one of the runs" do
        expect(result).to match_array(runs)
  ...
  context "when the second run overlaps the left edge of the first" do
    context "by 50%" do
      let(:runs) do
        [
          PDF::Reader2::TextRun.new(30, 700, 50, 12, "Hello"),
          PDF::Reader2::TextRun.new(5, 700, 50, 12, "Hello"),
        ]
      end
      it "returns only one of the runs" do
  ...
  context "when the second run overlaps the top edge of the first" do
    context "by 50%" do
      let(:runs) do
        [
          PDF::Reader2::TextRun.new(30, 700, 50, 12, "Hello"),
          PDF::Reader2::TextRun.new(30, 706, 50, 12, "Hello"),
        ]
      end
      it "returns only one of the runs" do
        expect(result).to match_array(runs.slice(0,1))
  ...
  context "when the second run overlaps the bottom edge of the first" do
    context "by 50%" do
      let(:runs) do
        [
          PDF::Reader2::TextRun.new(30, 700, 50, 12, "Hello"),
          PDF::Reader2::TextRun.new(30, 694, 50, 12, "Hello"),
        ]
      end
      it "returns only one of the runs" do
        expect(result).to match_array(runs.slice(0,1))
